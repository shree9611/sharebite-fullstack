...
async function listRequests(req, res) {
  ...
  try {
    ...
    console.info(
      `[${requestId}] listRequests start user=${req.user.id} role=${role} status=${rawStatusFilter || "none"} limit=${limit}`
    );
    ...
    // eslint-disable-next-line no-console
    console.info(`[${requestId}] listRequests complete rows=${payload.length} durationMs=${Date.now() - startedAt}`);
    // eslint-disable-next-line no-console
    console.info(`[${requestId}] listRequests payload-shape keys=${Object.keys(payload[0] || {}).join(",") || "none"}`);
